                        ...
                        <div class="col-lg-12 table-responsive m-b-0">
							<div class="amount_wrap">
                                <ul class="amount_list">
                                    <?php
                                        foreach($stock_state AS $key=>$value) {
                                    ?>
                                    <li>
                                        <dl class="amount_content">
                                            <dt>
                                                <h3><?=$value['material_name']?></h3>
                                            ...
                                            <dd class="text-indent">
                                                <span class="amount_tit">반품</span>
                                                <span class="amount_txt"><?=number_format($value['total_return_quantity']);?></span>
                                            </dd>
                                            <dd class="text-indent">
                                            ...
                                            <dd class="text-indent">
                                                <span class="amount_tit">예약</span>
                                                <span class="amount_txt"><?=number_format($value['total_schedule_quantity']);?></span>
                                            </dd>
                                        </dl>
                                    </li>
                                    <?php
                            ...
                            <table class="table table-bordered text-center dataTable" id="excelTable" style="min-width:1900px" >
                                <thead>
                                    <tr class="active">
                                        ...
                                        <th class="info sorting" data-order="material_name" style="width: 20%;">품목</th> 
                                        <th class="info sorting" data-order="company_name" style="width: 20%;" >협력업체명</th>                                        
                                        <th class="info sorting" data-order="quantity" >수량</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><?=number_format($value['quantity']);?> <?=$value['standard_info'];?></td>                     
                                        <td>
                                        ...
                                        <td>
                                            <?php
                                                if( $value['task_type'] == 'I' ) {
                                            ?>
                                            <button type="button" class="btn btn-sm btn-success waves-effect waves-light" onclick="showAddTaskLayer('U', '<?=$value['order_idx'];?>', '<?=$value['standard_info'];?>')" >사용</button>
                                            <button type="button" class="btn btn-sm btn-default waves-effect waves-light" onclick="showAddTaskLayer('R', '<?=$value['order_idx'];?>', '<?=$value['standard_info'];?>')" >반품</button>
                                            <button type="button" class="btn btn-sm btn-inverse waves-effect waves-light" onclick="showAddTaskLayer('D', '<?=$value['order_idx'];?>', '<?=$value['standard_info'];?>')" >폐기</button>
                                            <?php
                                                } else {
                                            ?>
        ...
        <div class="modal-content p-0 b-0">                                    
            <div class="panel panel-color panel-inverse m-0" style="width:800px" >
                <div class="panel-heading">
                    ...
                    <input type="hidden" id="task_order_idx" >
                </div>
...
<script>
    ...
    function showAddTaskLayer( arg_type, arg_order_idx, arg_standard_info ) {
        
        $('#current_quantity').text('');
        $('#use_quantity').val('');
        $('#net_contents').text(arg_standard_info);
        $('#task_type').val( arg_type );
        $('#task_order_idx').val( arg_order_idx );

        getStock( arg_order_idx );

        switch( arg_type ) {
            case 'U' : {
                $('#modal_title').text('원부자재 사용처리');
                break;
            }
            case 'R' : {
                $('#modal_title').text('원부자재 반품처리');
                break;
            }
            case 'D' : {
                $('#modal_title').text('원부자재 폐기처리');
                break;
            }
        }
    ...
    function getStock( arg_order_idx ) {

        jqueryAjaxCall({
            type : "post",
            url : '/materials/get_stocks_json',
            dataType : "json",
            paramData :{
                order_idx : arg_order_idx                          
            } ,
    ...
    function addTask(){
        ...
        if( viewFormValid.run( 'material_task_form' ) === true ) {
            ...
            jqueryAjaxCall({
                type : "post",
                url : '/materials/task_stocks_json',
                dataType : "json",
                paramData : {
                    order_idx : $('#task_order_idx').val()
                    ,task_type : task_type                          
