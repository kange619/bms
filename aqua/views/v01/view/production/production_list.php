                            ...
                            <table class="table table-bordered text-center dataTable" id="excelTable">
                                <thead>
                                    <tr class="active">
                                        ...
                                        <th class="info sorting" data-order="schedule_date" >생산 예정일</th>
                                        <th class="info sorting" data-order="expiration_date" >유통기한(일수)</th>
                                        <th class="info sorting" data-order="schedule_quantity" >생산 계획량</th>
                                        <th class="info sorting" data-order="reg_date"  >지시일시</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?=number_format($value['schedule_quantity']);?> <?=$value['product_unit_type'];?>
                                        ...
                                        <td><?=$production_status_arr[ $value['production_status'] ]?></td>
                                        <td>
                                            <?php
                                                if( $value['production_status'] == 'D') {
                                            ?>                                                                                  
                                            <button type="button" class="btn btn-sm btn-default waves-effect waves-light" onclick="alert('생산 완료처리되어 변경이 불가합니다.')" >변경불가</button>
                                            <?php
                                                } else {
                                            ?>
                                                <?php
                                                    if( $value['production_status'] == 'S' ) {
                                                ?>  
                                                <a href="./<?=$page_name?>_write?mode=edit&page=<?=$page?><?=$params?>&production_idx=<?=$value['production_idx'];?>">
                                                    <button type="button" class="btn btn-sm btn-purple waves-effect waves-light" >수정</button>
                                                </a>
                                                 <button type="button" class="btn btn-sm btn-purple waves-effect waves-light"  onclick="doneProc('<?=$value['production_idx'];?>')" >완료처리</button>
                                                <?php
                                                    } else {                                
                                                ?> 
                                               <button type="button" class="btn btn-sm btn-default waves-effect waves-light" >작업종료</button>
                                                <?php
                                                    }
                                                ?>
                                            <?php
                                        ...
                                        <td>
                                            <?php
                                                if( $value['production_status'] == 'S') {
                                            ?> 
                                            <button type="button" class="btn btn-danger" onclick="delProc('<?=$value['production_idx'];?>')">삭제</button>
                                            <?php
                                                } else {
                                            ?>
...
<script>
    ...
    function doneProc( arg_idx ) {
        if(confirm('해당 정보를 생산완료 처리하시겠습니까?') == true ){
            $('#mode').val('done');
            $('#production_idx').val(arg_idx);
            $('#list_form').submit();
        }
